...
the matching OOD run. For a randomly selected fraction of events, the post-decision correctness
...
def replay_sparse_feedback(
    *,
    ...
    for event_index, event in enumerate(events):
        ...
        totals["route_correct"] += route_correct
        totals["vote_correct"] += vote_correct
        totals["total"] += 1
    ...
    metrics = {
        **totals,
        "route_accuracy": totals["route_correct"] / total,
        "vote_accuracy": totals["vote_correct"] / total,
    }
    return {
        "metrics": metrics,
...
def _majority_metrics(events: Sequence[PreparedEvent]) -> dict[str, Any]:
    ...
    for event in events:
        ...
        event_correct = _frozen_vote_correctness(
            answer, list(event.canonical_answers), event.correctness
        )[0]
        num_correct += int(event_correct)
    return {
        "correct": num_correct,
        "total": len(events),
        "accuracy": num_correct / len(events),
    }


def _load_profile_phis(
    profile_name: str, *, offline_data: Path, num_events: int
) -> tuple[np.ndarray, dict[str, Any]]:
    profile = MODEL_RUNS[profile_name]
    output = profile.routing_output
    phi_path = output / "phis.npy"
    manifest_path = output / "phi_manifest.json"
    if not phi_path.is_file() or not manifest_path.is_file():
        raise FileNotFoundError(f"Missing OOD phi cache for {profile_name}")
    manifest = json.loads(manifest_path.read_text())
    expected = {
        "offline_data_sha256": _sha256_file(offline_data),
        "checkpoint_sha256": _sha256_file(profile.checkpoint / "sym_memory.pt"),
    ...
    if changed:
        raise ValueError(f"OOD phi cache does not match the requested run: {changed}")
    ...
    if phis.shape[0] != num_events or not np.isfinite(phis).all():
    ...
    for method in ("route", "vote"):
        ...
        for percent in FEEDBACK_PERCENTS:
            ...
            accuracies = np.asarray(
                [run[f"{method}_accuracy"] for run in selected],
                dtype=np.float64,
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("outputs/ood_feedback_sparsity")
    )
...
def main() -> None:
    ...
    for profile_name in PROFILES:
        ...
        profile = MODEL_RUNS[profile_name]
        phis, phi_manifest = _load_profile_phis(
            profile_name,
            offline_data=args.offline_data,
            num_events=len(events),
        )
        gamma = float(phi_manifest["gamma"])
        eta = float(phi_manifest["eta"])
        ...
        for seed in SEEDS:
            for percent in FEEDBACK_PERCENTS:
                ...
                row = {
                    ...
                    "vote_correct": metrics["vote_correct"],
                ...
                print(
                    ...
                    f"feedback={percent}% route={metrics['route_accuracy'] * 100:.2f}% "
                    f"vote={metrics['vote_accuracy'] * 100:.2f}%",
                    flush=True,
                )
        for method in ("route", "vote"):
            full_feedback = [
                run[f"{method}_correct"]
                for run in runs
        ...
        profile_results[profile_name] = {
            "profile": {
                ...
                "checkpoint_sha256": phi_manifest["checkpoint_sha256"],
    ...
    summary = {
        "status": "complete",
        "experiment": "ood_feedback_sparsity_direct_M",
