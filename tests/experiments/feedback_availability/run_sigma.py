...
def _metrics_complete(output: Path, expected_examples: int) -> bool:
    ...
    try:
        ...
        return False
    if int(metrics.get("num_samples", -1)) != expected_examples:
        return False
    return True
...
def _jobs(root: Path, offline_data: Path, python: Path) -> list[Job]:
    jobs: list[Job] = []
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("outputs/ood_sigma_feedback_sparsity")
    )
...
def main() -> None:
    ...
    expected_examples = sum(
        bool(line.strip()) for line in args.offline_data.read_text().splitlines()
    )
    args.output.mkdir(parents=True, exist_ok=True)

    pending = _jobs(args.output, args.offline_data, args.python)
    if args.force:
        runnable = pending
    else:
        runnable = [
            job
            for job in pending
            if not _metrics_complete(job.output, expected_examples)
        ]
    ...
    while runnable or running:
        ...
        for proc in list(running):
            ...
            if ret != 0 or not _metrics_complete(job.output, expected_examples):
