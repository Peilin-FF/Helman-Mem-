...
Sanity arm --ablate_memory: no steering -> must reproduce NO-WRITE.
  ...
  python -m tests.experiments.common.evaluate_sigma --config configs/symmetric_memory_candidate_yesno.yaml \
    --checkpoint outputs/sym_steer_06 --central_model models/Qwen3-0.6B \
    --offline_data data/counterfactual_3peer/cf_90.jsonl --output outputs/eval_sym/steer_cf_90
"""
...
def _load_shape_compatible(module, state_dict, label):
    ...
    This keeps train-3/test-5 evaluation usable by skipping runtime buffers whose
    peer-shaped dimensions do not belong to the learned encoder.
    """
...
def main():
    ...
    mem = SymmetricTrustMemory(num_peers=num_peers, rank=rank, task_types=task_types,
                               ...
                               proto_tau=proto_tau, device=device).to(device)
    ...
    # Runtime M/G buffers are rebuilt for the requested peer count. Shape filtering keeps
    # checkpoint loading explicit if an older checkpoint contains incompatible tensors.
    if ckpt is not None and (ckpt / "sym_memory.pt").exists():
        payload = torch.load(ckpt / "sym_memory.pt", map_location=device)
        # Published checkpoints predate the cleanup and contain an unused internal
        # G buffer and decay parameter. The runtime joint G is rebuilt below.
        mem_sd = {
            k: v for k, v in payload["mem"].items()
            if k not in ("M", "G", "_theta_joint")
        }
    ...
    with torch.no_grad():
        for rec_idx, rec in enumerate(records, start=1):
            ...
            if ablate:
                ...
                sigma_scores = [float(sigma_tensor[s]) for s in range(real)]
                scores = sigma_scores
            ...
            # Event-axis update after selection; sign is external correctness.
    ...
    print(f"[eval_sym/steer/{phi_mode}] {cfg['offline_data']}: accuracy={acc*100:.2f} over {total} "
          f"(ablate={ablate}, peer_mode={peer_mode}, "
