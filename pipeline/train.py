...
def train_jobs(plan) -> list:
    ...
    for arm, spec in arms(plan).items():
        ...
        args = " ".join(list(cfg.get("overrides", [])) + (list(cfg.get("tilt_overrides", [])) if spec.get("tilt") else []) + smoke)
        out = L.train_dir(arm)
        cmd = (f"EXP={arm} MODEL={init} OUT={out} TRAIN={train_pq} VAL={val_pq} bash training/scripts/train_grpo.sh {args} "
