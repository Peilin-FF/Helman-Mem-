...
Sanity arm --ablate_memory: no steering -> must reproduce NO-WRITE.
  ...
  PYTHONPATH=. python eval_symmetric_memory.py --config configs/symmetric_memory_candidate_yesno.yaml \
...
def main():
    ...
    # checkpoint loading explicit if a non-published checkpoint contains incompatible tensors.
    if ckpt is not None and (ckpt / "sym_memory.pt").exists():
        ...
        # G buffer and decay parameter. The paper's joint G is rebuilt below.
        mem_sd = {
