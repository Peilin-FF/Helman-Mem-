...
CF_SPLITS = ("p0", "p50", "p70", "p90")
CF_STREAM_SIZE = 2_685
CF_STREAM_SHA256 = {
    "p0": "67b8bbacacbdbb4b772fe438d78db473f781d857fb71139c367e66cc35353f01",
    "p50": "8b2fc63caa0aec24db4d1f4d53501480b9f6ab44206447a929eb226318f0c211",
    "p70": "e27ba97af72e9fb4fb71392a87a79420dd9f06f3c629ae7d179633ce622508ff",
    "p90": "57bf0ebef914f7caf48d5dbfa8ed5aef2af3cceb704d733ab562633785b167ec",
}
...
def _load_cf_streams(
    ...
    for split in CF_SPLITS:
        ...
        if strict and file_sha != CF_STREAM_SHA256[split]:
            raise ValueError(
                f"{split} SHA256 changed: {file_sha} != {CF_STREAM_SHA256[split]}"
            )
        if strict and len(records) != CF_STREAM_SIZE:
            raise ValueError(
                f"{split} size changed: {len(records)} != {CF_STREAM_SIZE}"
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-dir", type=Path, default=Path("data/CF_unified"))
    parser.add_argument("--output", type=Path, default=None)
    parser.add_argument("--device", default="cuda:0")
    parser.add_argument("--dtype", default="bfloat16")
    parser.add_argument("--max-length", type=int, default=2048)
    parser.add_argument("--allow-noncanonical-streams", action="store_true")
...
def main() -> None:
    ...
    strict = not bool(args.allow_noncanonical_streams)
    streams, stream_provenance = _load_cf_streams(args.data_dir, strict=strict)
    p0_records = streams["p0"]
    ...
    if strict and checkpoint_sha != profile.checkpoint_sha256:
        raise ValueError("Registered Sigma checkpoint changed")
    if strict and train_config_sha != profile.train_config_sha256:
        raise ValueError("Registered Sigma train config changed")
    ...
    manifest_base = {
        ...
        "p0_sha256": stream_provenance["p0"]["sha256"],
        "p0_ordered_uid_sha256": stream_provenance["p0"]["ordered_uid_sha256"],
        "question_map_sha256": stream_provenance["p0"]["question_map_sha256"],
    ...
    if phis is None:
        ...
        if not math.isclose(
            float(encoder_metadata["gamma"]), profile.gamma, rel_tol=0.0, abs_tol=1e-8
        ) or not math.isclose(
            float(encoder_metadata["eta"]), profile.eta, rel_tol=0.0, abs_tol=1e-8
        ):
            raise ValueError("Loaded checkpoint gamma/eta differ from registered profile")
    ...
    summary = {
        "status": "complete" if strict else "diagnostic",
