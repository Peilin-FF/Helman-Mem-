...
Sources (graded, 3 peers = gemma/phi/qwen-coder):
  math : data/mixed_train_sources/gsm8k.graded.jsonl
  rag  : data/mixed_train_sources/squad.graded.jsonl
  code : data/mixed_train_sources/apps.graded.jsonl

These train sources are disjoint from the CF test streams by construction
...
normalize both into correctness_by_peer so the Sigma trainer reads one field.

Balancing: --per_task N caps each task to N records (after seeded shuffle). Default 1000.
Output: data/mixed_train/train.jsonl (shuffled across tasks, seeded)
"""
...
TEST_STREAMS = [
    f"data/counterfactual_3peer/{split}.jsonl"
    for split in ("cf_0", "cf_50", "cf_70", "cf_90")
]
MATH_SRC = "data/mixed_train_sources/gsm8k.graded.jsonl"
RAG_SRC = "data/mixed_train_sources/squad.graded.jsonl"
CODE_SRC = "data/mixed_train_sources/apps.graded.jsonl"
PEERS = ("peer_0", "peer_1", "peer_2")
OUT = "data/mixed_train/train.jsonl"
