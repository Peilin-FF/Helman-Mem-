...
Writes data/mixed_train_sources/apps_solvable_ids.json (a list of APPS ids).
load_apps() then filters
to this set automatically (see APPS_SOLVABLE_IDS). Re-run only if the slice filter
(stdin-only / APPS_MAX_SOL_CHARS) changes.

    FEEDBACK_CODE_EXEC_ALLOW=1 python -m data.builders.mixed_train.build_apps_solvable_ids
"""
...
from data.builders.common.benchmark_datasets import (
    APPS_MAX_SOL_CHARS,
    _dataset_path,
    _io_text,
)
from data.builders.common.code_grading import check_io


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Verify APPS reference solutions pass check_io.")
    p.add_argument("--split", default="train")
    p.add_argument("--output", default="data/mixed_train_sources/apps_solvable_ids.json")
