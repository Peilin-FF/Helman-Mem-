...
These feed ``data.builders.common.generate_setting_a_peers`` (peer generation)
and, for code, ``data.builders.common.score_code_peers`` (offline pass@1).
...
_HF_DATA_DIR = os.environ.get(
    "HF_DATA_DIR", str(_REPO_ROOT / ".cache" / "huggingface" / "datasets")
)
_LOCAL_DIRS = {
...
# check_io grader (built once by data.builders.mixed_train.build_apps_solvable_ids).
...
APPS_SOLVABLE_IDS = os.environ.get(
    "APPS_SOLVABLE_IDS",
    "data/mixed_train_sources/apps_solvable_ids.json",
)
...
def load_eval_dataset(
    ...
    *,
    ...
    Math datasets are not here; use data.builders.common.math_datasets instead and
