...
OUT_ROOT="${OUT_ROOT:-outputs/eval_cf_peer3_llama_by_center}"
LOGD="${LOGD:-logs/eval_cf_peer3_llama_by_center}"
...
bootstrap_existing_q3_06() {
  local src_root="outputs/eval_cf_unified_peer_counts_llama_bitcpm_shifted_q3_0.6b/peers4"
  local tag="q3_0.6b" split arm src dst
  for split in "${SPLITS[@]}"; do
    for arm in center sigma; do
      src="$src_root/${arm}_${split}"
      dst="$(out_dir "$tag" "$arm" "$split")"
      if [[ -f "$src/eval_metrics.json" && ! -f "$dst/eval_metrics.json" ]]; then
        mkdir -p "$(dirname "$dst")"
        cp -a "$src" "$dst"
        echo "[$(date '+%F %T')] copied existing $tag $arm $split from $src"
      fi
    done
  done
}
...
echo "[$(date '+%F %T')] start peer3/Llama CF@ scheduler"
echo "data=$DATA_DIR"
echo "out=$OUT_ROOT"
...
while true; do
  ...
  for entry in "${MODELS[@]}"; do
    ...
    for split in "${SPLITS[@]}"; do
      for arm in center sigma; do
        task_done "$tag" "$arm" "$split" && continue
        key="$tag|$arm|$split"
  ...
  for entry in "${MODELS[@]}"; do
    ...
    for split in "${SPLITS[@]}"; do
      for arm in center sigma; do
        task_done "$tag" "$arm" "$split" || remaining=$((remaining + 1))
      done
    done
  done
...
echo "[$(date '+%F %T')] all peer3/Llama CF@ evals complete"
