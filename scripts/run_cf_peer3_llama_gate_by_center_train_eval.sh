#!/usr/bin/env bash
set -Euo pipefail

cd "$(dirname "$0")/.."

source scripts/gpu_lock_lib.sh

export PYTHONPATH=".:${PYTHONPATH:-}"
export HF_DATA_DIR="${HF_DATA_DIR:-/mnt/data/peilin/HF_DATA}"
export HF_DATASETS_OFFLINE="${HF_DATASETS_OFFLINE:-1}"
export HF_HUB_OFFLINE="${HF_HUB_OFFLINE:-1}"
export TRANSFORMERS_OFFLINE="${TRANSFORMERS_OFFLINE:-1}"
export FEEDBACK_CODE_EXEC_ALLOW="${FEEDBACK_CODE_EXEC_ALLOW:-1}"

PY_SIGMA="${PY_SIGMA:-/home/peilin/miniconda3/envs/sigma/bin/python}"
PY_SIGMA35="${PY_SIGMA35:-/home/peilin/miniconda3/envs/sigma3_5/bin/python}"
HFM="${HFM:-/mnt/data/peilin/HF_MODEL}"
TRAIN="${TRAIN:-data/mixed_train_labeled.jsonl}"
DATA_DIR="${DATA_DIR:-data/CF_unified_peer_counts_llama_bitcpm_shifted/peers4}"
CONFIG="${CONFIG:-configs/symmetric_memory_candidate_yesno.yaml}"
GATE="${GATE:-margin}"
THRESHOLD="${THRESHOLD:-0.125}"
OUT_ROOT="${OUT_ROOT:-outputs/eval_cf_peer3_llama_gate_${GATE}${THRESHOLD}_by_center}"
LOGD="${LOGD:-logs/eval_cf_peer3_llama_gate_${GATE}${THRESHOLD}_by_center}"
MAX_LENGTH="${MAX_LENGTH:-8192}"
POLL_SECONDS="${POLL_SECONDS:-60}"
GPU_MAX_USED_MB="${GPU_MAX_USED_MB:-1000}"
GPU_MAX_UTIL="${GPU_MAX_UTIL:-10}"

mkdir -p "$OUT_ROOT" "$LOGD"

if [[ -n "${GPUS:-}" ]]; then
  read -r -a ALLOWED_GPUS <<< "$GPUS"
else
  mapfile -t ALLOWED_GPUS < <(nvidia-smi --query-gpu=index --format=csv,noheader,nounits)
fi
read -r -a SPLITS <<< "${SPLITS:-p0 p50 p70 p90}"

MODELS=(
  "q3_0.6b|$HFM/Qwen3-0.6B|$PY_SIGMA"
  "q3_4b|$HFM/Qwen3-4B|$PY_SIGMA"
  "q3_8b|$HFM/Qwen3-8B|$PY_SIGMA"
  "q35_4b|$HFM/Qwen3.5-4B|$PY_SIGMA35"
  "q35_9b|$HFM/Qwen3.5-9B|$PY_SIGMA35"
)

declare -A MODEL_PATH PYTHON_BIN
for entry in "${MODELS[@]}"; do
  IFS='|' read -r tag model pybin <<< "$entry"
  MODEL_PATH["$tag"]="$model"
  PYTHON_BIN["$tag"]="$pybin"
done

ckpt_dir() {
  local tag="$1"
  echo "outputs/sigma_candidate_yesno_gate_${GATE}${THRESHOLD}_${tag}/proto"
}

eval_out_dir() {
  local tag="$1" split="$2"
  echo "$OUT_ROOT/$tag/gate_${split}"
}

train_done() {
  local tag="$1"
  [[ -f "$(ckpt_dir "$tag")/sym_memory.pt" ]]
}

eval_done() {
  local tag="$1" split="$2"
  [[ -f "$(eval_out_dir "$tag" "$split")/eval_metrics.json" ]]
}

in_allowed_gpus() {
  local gpu="$1" x
  for x in "${ALLOWED_GPUS[@]}"; do
    [[ "$x" == "$gpu" ]] && return 0
  done
  return 1
}

declare -A GPU_TASK PID_TASK PID_LOG TASK_PID

find_free_gpu() {
  local task_key="$1"
  local idx used util
  while IFS=',' read -r idx used util; do
    idx="${idx//[[:space:]]/}"
    used="${used//[[:space:]]/}"
    util="${util//[[:space:]]/}"
    in_allowed_gpus "$idx" || continue
    [[ -n "${GPU_TASK[$idx]:-}" ]] && continue
    if [[ "$used" -le "$GPU_MAX_USED_MB" && "$util" -le "$GPU_MAX_UTIL" ]] && gpu_try_claim "$idx" "$task_key"; then
      echo "$idx"
      return 0
    fi
  done < <(nvidia-smi --query-gpu=index,memory.used,utilization.gpu --format=csv,noheader,nounits)
  return 1
}

launch_train() {
  local tag="$1" gpu="$2"
  local key="train|$tag|" out log
  out="$(ckpt_dir "$tag")"
  log="$LOGD/train_${tag}_gpu${gpu}.log"
  mkdir -p "$out"
  echo "[$(date '+%F %T')] launch gpu=$gpu $key gate=${GATE}${THRESHOLD} -> $out"
  CUDA_VISIBLE_DEVICES="$gpu" "${PYTHON_BIN[$tag]}" -u train_symmetric_memory.py \
    --config "$CONFIG" \
    --central_model "${MODEL_PATH[$tag]}" \
    --offline_data "$TRAIN" \
    --output_dir "$out" \
    --score_mode candidate_yesno \
    --peer_mode joint \
    --per_peer_decay off \
    --diff_write on \
    --confusion_gate "$GATE" \
    --confusion_threshold "$THRESHOLD" \
    > "$log" 2>&1 &
  local pid=$!
  PID_TASK["$pid"]="$key"
  PID_LOG["$pid"]="$log"
  TASK_PID["$key"]="$pid"
  GPU_TASK["$gpu"]="$key"
  gpu_update_claim_pid "$gpu" "$pid" "$key"
}

launch_eval() {
  local tag="$1" split="$2" gpu="$3"
  local key="eval|$tag|$split" out log
  out="$(eval_out_dir "$tag" "$split")"
  log="$LOGD/eval_${tag}_gate_${split}_gpu${gpu}.log"
  mkdir -p "$out"
  echo "[$(date '+%F %T')] launch gpu=$gpu $key -> $out"
  CUDA_VISIBLE_DEVICES="$gpu" "${PYTHON_BIN[$tag]}" -u eval_symmetric_memory.py \
    --config "$CONFIG" \
    --checkpoint "$(ckpt_dir "$tag")" \
    --central_model "${MODEL_PATH[$tag]}" \
    --num_peers 4 \
    --max_length "$MAX_LENGTH" \
    --offline_data "$DATA_DIR/${split}.jsonl" \
    --output "$out" \
    --score_mode candidate_yesno \
    --peer_mode joint \
    --per_peer_decay off \
    --confusion_gate "$GATE" \
    --confusion_threshold "$THRESHOLD" \
    > "$log" 2>&1 &
  local pid=$!
  PID_TASK["$pid"]="$key"
  PID_LOG["$pid"]="$log"
  TASK_PID["$key"]="$pid"
  GPU_TASK["$gpu"]="$key"
  gpu_update_claim_pid "$gpu" "$pid" "$key"
}

reap_finished() {
  local pid key gpu rc
  set +u
  local pids=("${!PID_TASK[@]}")
  set -u
  for pid in "${pids[@]}"; do
    if kill -0 "$pid" 2>/dev/null; then
      continue
    fi
    key="${PID_TASK[$pid]}"
    rc=0
    wait "$pid" || rc=$?
    gpu=""
    for g in "${!GPU_TASK[@]}"; do
      if [[ "${GPU_TASK[$g]}" == "$key" ]]; then
        gpu="$g"
        unset 'GPU_TASK[$g]'
        gpu_release_claim "$g" "$key"
        break
      fi
    done
    echo "[$(date '+%F %T')] finish rc=$rc gpu=${gpu:-?} $key log=${PID_LOG[$pid]}"
    if [[ "$rc" -ne 0 ]]; then
      echo "[fail] $key failed; see ${PID_LOG[$pid]}" >&2
      exit "$rc"
    fi
    unset 'PID_TASK[$pid]' 'PID_LOG[$pid]'
  done
}

active_count() {
  set +u
  echo "${#PID_TASK[@]}"
  set -u
}

validate_inputs() {
  local split tag
  [[ -f "$TRAIN" ]] || { echo "[fail] missing train data: $TRAIN" >&2; exit 1; }
  for split in "${SPLITS[@]}"; do
    [[ -f "$DATA_DIR/${split}.jsonl" ]] || { echo "[fail] missing $DATA_DIR/${split}.jsonl" >&2; exit 1; }
  done
  for tag in "${!MODEL_PATH[@]}"; do
    [[ -d "${MODEL_PATH[$tag]}" ]] || { echo "[fail] missing model ${MODEL_PATH[$tag]}" >&2; exit 1; }
    [[ -x "${PYTHON_BIN[$tag]}" ]] || { echo "[fail] missing python ${PYTHON_BIN[$tag]}" >&2; exit 1; }
  done
}

remaining_count() {
  local remaining=0 tag split
  for entry in "${MODELS[@]}"; do
    IFS='|' read -r tag _model _pybin <<< "$entry"
    train_done "$tag" || remaining=$((remaining + 1))
    if train_done "$tag"; then
      for split in "${SPLITS[@]}"; do
        eval_done "$tag" "$split" || remaining=$((remaining + 1))
      done
    else
      remaining=$((remaining + ${#SPLITS[@]}))
    fi
  done
  echo "$remaining"
}

validate_inputs

echo "[$(date '+%F %T')] start peer3/Llama gate scheduler"
echo "gate=$GATE threshold=$THRESHOLD"
echo "train=$TRAIN"
echo "data=$DATA_DIR"
echo "out=$OUT_ROOT"
echo "allowed_gpus=${ALLOWED_GPUS[*]} idle=(mem<=${GPU_MAX_USED_MB}MiB util<=${GPU_MAX_UTIL}%) poll=${POLL_SECONDS}s"

while true; do
  reap_finished
  launched=0

  for entry in "${MODELS[@]}"; do
    IFS='|' read -r tag _model _pybin <<< "$entry"
    if ! train_done "$tag"; then
      key="train|$tag|"
      [[ -n "${TASK_PID[$key]:-}" ]] && continue
      if gpu="$(find_free_gpu "$key")"; then
        launch_train "$tag" "$gpu"
        launched=1
      else
        break
      fi
    fi
  done

  for entry in "${MODELS[@]}"; do
    IFS='|' read -r tag _model _pybin <<< "$entry"
    train_done "$tag" || continue
    for split in "${SPLITS[@]}"; do
      eval_done "$tag" "$split" && continue
      key="eval|$tag|$split"
      [[ -n "${TASK_PID[$key]:-}" ]] && continue
      if gpu="$(find_free_gpu "$key")"; then
        launch_eval "$tag" "$split" "$gpu"
        launched=1
      else
        break 2
      fi
    done
  done

  remaining="$(remaining_count)"
  echo "[$(date '+%F %T')] status remaining=$remaining active=$(active_count)"
  [[ "$remaining" -eq 0 && "$(active_count)" -eq 0 ]] && break
  if [[ "$launched" -eq 0 ]]; then
    sleep "$POLL_SECONDS"
  fi
done

echo "[$(date '+%F %T')] all peer3/Llama gate train/eval jobs complete"
