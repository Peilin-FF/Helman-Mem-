...
def main() -> None:
    ...
    findings = []
    def drop(s_, k):
        a, b = acc(s_, k), sw(s_, k)
        return None if a is None or b is None else a - b
    findings.append(f"<li><b>Words barely steer; structure does.</b> Describing the record in words (ranked, with the model's own record, with a verification procedure) leaves accuracy where the number left it (OOD {f1(acc('ood', 'number'), 1)} → {f1(acc('ood', 'ordinal'), 1)} / {f1(acc('ood', 'self_note'), 1)} / {f1(acc('ood', 'verify'), 1)}; in-distribution {f1(acc('indist', 'number'), 1)} → {f1(acc('indist', 'ordinal'), 1)} / {f1(acc('indist', 'self_note'), 1)} / {f1(acc('indist', 'verify'), 1)}). The swapped control moves by {f1(drop('ood', 'ordinal'), 1)} / {f1(drop('ood', 'verify'), 1)} points on OOD and {f1(drop('indist', 'ordinal'), 1)} / {f1(drop('indist', 'verify'), 1)} in-distribution: in-domain, where the model is less sure of the content, the words do shift which peer it follows, but not far enough to raise accuracy. Removing or re-ordering what the model sees does steer: only-the-favourite {f1(acc('ood', 'favourite'), 1)} / {f1(acc('indist', 'favourite'), 1)}, unreliable peers withheld {f1(acc('ood', 'filtered'), 1)} / {f1(acc('indist', 'filtered'), 1)}, sorted {f1(acc('ood', 'sorted'), 1)} / {f1(acc('indist', 'sorted'), 1)}, and these lose {f1((acc('ood', 'favourite') or 0) - (sw('ood', 'favourite') or 0), 1)} / {f1((acc('indist', 'favourite') or 0) - (sw('indist', 'favourite') or 0), 1)} points when the record is swapped, which is what a steered model must do. This matches the literature: credibility text is ignored by untrained models (CAG), while position and majority size drive conformity (the conformity studies), so changing position and majority is the lever.</li>")
    ...
    at, ats, a3, a3s = o.get("attn_g1"), o.get("attn_g1_swapped"), o.get("attn_g3"), o.get("attn_g3_swapped")
    if at:
        i3, i3s = i_.get("attn_g3"), i_.get("attn_g3_swapped")
        findings.append(f"<li><b>Attention-level steering is the strongest in-model method, and it is the same Bayes rule applied to attention.</b> Attention over the prompt is a distribution over source tokens; multiplying the weights over peer i's tokens by its credibility and renormalising, softmax(scores + γ·log c<sub>i</sub>), is a Bayesian tilt of that distribution by a prior over sources, with γ the strength of the prior (CrAM's rule). With no reliability text in the prompt at all, γ = 1 gives {at['accuracy']:.2f} OOD (swapped {f1(ats['accuracy'] if ats else None, 2)}) and {f1(i_.get('attn_g1', {}).get('accuracy'), 2)} in-distribution (swapped {f1(i_.get('attn_g1_swapped', {}).get('accuracy'), 2)}); γ = 3 gives {f1(a3['accuracy'] if a3 else None, 2)} OOD (swapped {f1(a3s['accuracy'] if a3s else None, 2)}) and {f1(i3['accuracy'] if i3 else None, 2)} in-distribution (swapped {f1(i3s['accuracy'] if i3s else None, 2)}). On the OOD selection events the γ = 3 model is right {f1(a3.get('anycorrect_model_right') if a3 else None)}% and follows the favourite {f1(a3.get('anycorrect_model_follows_favourite') if a3 else None)}%, the favourite's own rate; it beats every prompt variant on OOD and still sits below the model alone, because the peers are in the prompt. The bias is on all heads and one strength; CrAM selects heads by causal tracing, the natural refinement, and γ can be set from the record's own uncertainty (a sharper posterior, a stronger tilt).</li>")
