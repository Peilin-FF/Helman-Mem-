...
What improved, reading {f1(frozen_in['by']['rag'], 0)} → {f1(min(v for v in (ev(mem, 'ev_in', 'rag'), ev(peers, 'ev_in', 'rag'), ev(lab, 'ev_in', 'rag')) if v), 0)}–{f1(max(v for v in (ev(mem, 'ev_in', 'rag'), ev(peers, 'ev_in', 'rag'), ev(lab, 'ev_in', 'rag')) if v), 0)}, is format and extraction learning that plain RLVR also gets ({f1(ev(plain, 'ev_in', 'rag'), 0)}); what got worse, code and BIG-Bench Hard, is the imported peer style.</p>"""
    ...
    problems = [
        ...
        li(6, "Thinking off caps the reasoning that the OOD tasks need.",
           f"The frozen model with thinking on scores {f1(frozen_ood_think['acc'] if frozen_ood_think else None, 2)} on OOD against {f1(frozen_ood['acc'], 2)} without, above the best non-thinking RL model ({f1(ev(plain, 'ev_ood'), 2)}). The memory run with thinking on keeps its reasoning length after 70 steps (2.6k characters per trace against 2.5k frozen), gains the same reading improvement as the non-thinking runs and loses nothing on OOD ({f1(ev(think[0], 'ev_ood') if think else None, 2)}); plain RLVR with thinking reaches {f1(ev(think[1], 'ev_in') if len(think) > 1 else None, 2)} in-distribution and {f1(ev(think[1], 'ev_ood') if len(think) > 1 else None, 2)} OOD after the same 70 steps, so with thinking on the three models are within 0.4 points of each other on OOD. On the probe slice thinking lifts the frozen model with peers from {f1(fo.get('accuracy'), 2)} to {f1(fto.get('accuracy'), 2)}; with thinking the note is quoted in {P((fto.get('thinking') or {}).get('mentions_notes'), 0)} of OOD traces and {P((pi.get('frozen_think_notes', {}).get('thinking') or {}).get('mentions_notes'), 0)} of in-distribution traces, and still not acted on (in-distribution follow rates {f1(pi.get('frozen_think_notes', {}).get('follow_top_informative'))} / {f1(pi.get('frozen_think_nonotes', {}).get('follow_top_informative'))} / {f1(pi.get('frozen_think_swapped', {}).get('follow_low_informative'))}% shown / hidden / swapped).",
...
{('<div class="tbl"><table>' + ''.join(t3) + '</table></div><p class="sub">Along the stream: the memory starts from flat notes and sharpens as evidence accumulates; the task-conditioned running mean improves too but stays below it throughout.</p>') if t3 else ''}"""
    ...
    for title, a, b, c in (("frozen, OOD slice", fo, fn, fs), ("trained with notes (step 276), OOD slice", to, tn, ts), ("frozen, in-distribution slice", io_, in_, is_), ("trained with notes (step 276), in-distribution slice", ito, itn, its),
                           ...
                           ("frozen + thinking, in-distribution slice", pi.get("frozen_think_notes", {}), pi.get("frozen_think_nonotes", {}), pi.get("frozen_think_swapped", {})),
                           ("trained with notes + thinking (step 70), in-distribution slice", pi.get("memthink_notes", {}), pi.get("memthink_nonotes", {}), pi.get("memthink_swapped", {}))):
        if a:
