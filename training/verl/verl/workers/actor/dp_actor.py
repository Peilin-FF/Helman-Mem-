...
try:   # kalman: the memory's attention tilt (additive mask) for the non-rmpad forward
    from feedback_state import attn_bias as _kalman_attn_bias
except ImportError:   # pragma: no cover
    _kalman_attn_bias = None
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch, temperature, calculate_entropy=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            entropy = None
            if _kalman_attn_bias is not None and _kalman_attn_bias.HF_STEER is not None:   # kalman: tilt of this micro-batch (prompt part; response tokens get 0)
                ...
                _kalman_attn_bias.HF_STEER.bias = None if ab is None else torch.nn.functional.pad(ab.to(device=input_ids.device, dtype=torch.float32), (0, seqlen - ab.shape[1]))
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels:
                    extra_args["temperature"] = temperature
                # kalman: trim the padding shared by the whole micro-batch (left pads of the prompts, right pads of the
                ...
                if trim:
                    ...
                    if _kalman_attn_bias is not None and _kalman_attn_bias.HF_STEER is not None and _kalman_attn_bias.HF_STEER.bias is not None:
                        _kalman_attn_bias.HF_STEER.bias = _kalman_attn_bias.HF_STEER.bias[:, s:e]
                else:
                ...
                if self.use_fused_kernels:
                    ...
                    if os.environ.get("KALMAN_TRIM_CHECK") and not getattr(self, "_trim_checked", False):   # one-off: trimmed == padded forward
                        self._trim_checked = True
                        with torch.no_grad():
                            if _kalman_attn_bias is not None and _kalman_attn_bias.HF_STEER is not None and _kalman_attn_bias.HF_STEER.bias is not None:
                                _kalman_attn_bias.HF_STEER.bias = torch.nn.functional.pad(_kalman_attn_bias.HF_STEER.bias, (s, seqlen - e))
                            ...
                            print(f"[kalman-trim-check] window {s}:{e} of {seqlen} (kept {kept}/{response_length} response positions); |trimmed - padded| log-prob max {diff.max().item():.5f} mean {diff.mean().item():.6f}", flush=True)
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        if "attn_bias" in data.batch.keys():   # kalman
