# question-only arm for run 3/3b: identical schedule and settings, the question alone in every training prompt (no peers, no memory);
# tested afterwards in the same four conditions as ours and the control (peers + tilt / peers / alone / swapped tilt)
set -u
...
for f in ["$D/full_solo.parquet", "$D/val_solo.parquet", "$D/full_peers.parquet"]:
    df = pd.read_parquet(f); print(f, len(df), "rows; peer blocks in prompt:", int(df["prompt"].astype(str).str.contains("Peer 1").sum()))
PY
echo "=== solo phase 1 (40 steps, seed 1) on GPUs 0-7: $(date)"
GPUS=0,1,2,3,4,5,6,7 EXP=solo3_q TRAIN=$D/full_solo.parquet VAL=$D/val_solo.parquet bash training/scripts/train_grpo.sh $COMMON trainer.total_training_steps=40 > $L/solo3_q.train.out 2>&1
echo "=== phase 1 finished (exit $?): $(date)"
CK0=outputs/rl/solo3_q/hf/global_step_40; ls $CK0/config.json
echo "=== solo phase 2 (from step 40, seed 2, full epoch) on GPUs 0-7: $(date)"
GPUS=0,1,2,3,4,5,6,7 EXP=solo3b_q MODEL=$CK0 TRAIN=$D/full_solo.parquet VAL=$D/val_solo.parquet bash training/scripts/train_grpo.sh $COMMON +data.seed=2 > $L/solo3b_q.train.out 2>&1
echo "=== phase 2 finished (exit $?): $(date)"
CK=$(last_ck solo3b_q); echo "solo checkpoint $CK"
echo "=== tests: solo arm: $(date)"; tests $CK
