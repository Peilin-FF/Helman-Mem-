# Other central-model families under the memory (docs section 23). Each family is tested with its OWN memory: the record's
# address comes from that family's model (launch_family_memory.sh builds the features, the record and the prompt files
# under outputs/gen/<tag>/), never from the Qwen3-4B judge (user, 2026-09-11). ADDR=q3_4b evaluates the same central
# model on the Qwen3-4B-addressed prompt files instead (a comparison of the two records; results get the suffix _q3addr).
# Per model and stream, three conditions:
...
O=outputs/gen/families; L=logs; mkdir -p $L $O
ADDR=${ADDR:-own}; sfx=""; [ $ADDR = q3_4b ] && sfx=_q3addr
...
for m in $models; do
  ...
  [ $ADDR = q3_4b ] || for st in $streams; do pf=$st; [ $st = oodfull ] && pf=ood
    [ -f outputs/gen/$m/prompts_${pf}6_probe.jsonl ] || { echo "no own-memory prompt file outputs/gen/$m/prompts_${pf}6_probe.jsonl for $m: run launch_family_memory.sh all $m first (or ADDR=q3_4b for the Qwen3-4B-addressed comparison)"; exit 1; }
  done
done
...
one () {   # gpu, model tag, stream, condition
  ...
  CUDA_VISIBLE_DEVICES=$gpu python -m tests.experiments.common.evaluate_memory_generator --central_model ${MODEL[$tag]} --engine vllm \
      --gpu_memory_utilization ${UTIL:-0.85} --thinking off --max_new_tokens 768 $extra_all \
