...
says they are more reliable on questions like this one and they agree.
  ...
  PYTHONPATH=. python -m analysis.self_vote_sim --model q3_4b --stream indist6 --order shuffled0 \
      --prompts outputs/record/q3_4b/indist6/shuffled0.fit-train6.jsonl --solo outputs/eval/q3_4b/indist6/solo
"""
...
def main() -> None:
    ...
    ap.add_argument("--stream", default="indist6")
    ap.add_argument("--fit-stream", default="train6")
    ...
    fit = feature_stream(args.fit_stream, args.model)
    proj_q = Projection(fit.sem, args.dim, device)
    fs = feature_stream(args.stream, args.model)
