...
weights logit(p) (reduces to route when no two candidates agree).
  ...
  PYTHONPATH=. python -m analysis.memsim --model q3_4b --streams ood6 indist6 train6 --dim 256 --device cuda:0
"""
...
from pipeline.layout import feature_stream
...
def main() -> None:
    ...
    ap.add_argument("--streams", nargs="+", default=["ood6", "indist6", "train6"])
    ap.add_argument("--fit-stream", default="train6", help="unlabeled stream used to fit the PCA projections")
    ...
    t0 = time.time()
    fit = feature_stream(args.fit_stream, args.model, limit=args.limit)
    ...
    for name in args.streams:
        sf = fit_sf if name == args.fit_stream else mk(feature_stream(name, args.model, limit=args.limit))
