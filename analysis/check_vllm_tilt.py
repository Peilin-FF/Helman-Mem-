...
tilt (should be bf16 noise, ~1e-2) and how far from HF without it (should be clearly larger), plus greedy agreement.
  ...
  CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python -m analysis.check_vllm_tilt --stage vllm --central_model /models/Qwen3-4B --out outputs/analysis/tilt_check
  CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python -m analysis.check_vllm_tilt --stage hf --central_model /models/Qwen3-4B --out outputs/analysis/tilt_check
"""
...
def parse_args():
    ...
    p.add_argument("--prompts", type=Path, default=Path("outputs/record/q3_4b/indist6/shuffled0.fit-train6.jsonl"))
